...
4. **Auto-classify and suggest enrichment:** Run the classifier against the issue title and body:
   ```bash
   ENRICH_TIER=$(bash ~/.claude/scripts/enrich-classify.sh "<ISSUE_TITLE>" "<ISSUE_BODY>")
   ```
   Use the clean title (from step 2) as `ISSUE_TITLE` and the original arguments text as `ISSUE_BODY`. Store the result as `ENRICH_TIER`. Do NOT block or prompt — just capture the tier silently for use in the confirmation message (step 7).
5. **Brief interview:** Ask the user 1-2 quick follow-up questions using AskUserQuestion:
...
6. **Enrich if answered:** If the user provides acceptance criteria (not "Skip"), edit the issue body to replace the `- [ ] TBD` placeholder with the user's criteria via `gh issue edit <N> --body "<updated body>"`. Read the current body first, then substitute the TBD line.
7. **Confirm** to the user with the issue URL, clean title, and any cross-references found. If `ENRICH_TIER` is `medium` or `complex`, append an enrichment suggestion to the confirmation:
   - For `medium`: "This issue was classified as **medium** complexity. Run `/backlog enrich #N` to auto-generate acceptance criteria and identify affected files."
   - For `complex`: "This issue was classified as **complex**. Run `/backlog enrich #N` to run deep research, generate a PRD skeleton, and identify affected files."
   - For `simple`: no enrichment mention — keep the confirmation clean.
